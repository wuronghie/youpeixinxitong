...
export default {
	...
	computed: {
		...
		canTrialRefund() {
			// 试课退款入口：课程已支付，状态为 confirmed / in_progress，尚未评价
	...
	methods: {
		async loadDetail() {
			...
			// 保存当前支付状态（如果已支付）
			const currentParentPaid = this.appointment?.parent_paid
			...
			try {
				...
				if (res.code === 0 && res.data) {
					...
					this.appointment = {
						...
						houry_rate: data.teacher_info?.hourly_rate,
						hourly_rate: data.teacher_info?.hourly_rate,

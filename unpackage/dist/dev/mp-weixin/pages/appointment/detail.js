  ...
  methods: {
    async loadDetail() {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/appointment/detail.vue:420", "获取预约详情失败:", error);
        ...
        common_vendor.index.showActionSheet({
          ...
          success: async (res) => {
            ...
            try {
              ...
              common_vendor.index.__f__("error", "at pages/appointment/detail.vue:677", "试算优惠券失败:", e);
              common_vendor.index.showToast({ title: "优惠券试算失败，请稍后重试", icon: "none" });
            }
          }
        });
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/appointment/detail.vue:683", "打开优惠券选择失败:", error);
...
确认支付？试课满意后可继续安排正式课程。`;
          ...
          try {
            ...
            if (payComponent && typeof payComponent.open === "function") {
              ...
              try {
                ...
                common_vendor.index.__f__("warn", "at pages/appointment/detail.vue:784", "uni-pay 组件调用失败，降级到模拟支付:", error);
              }
            }
            common_vendor.index.hideLoading();
            ...
            common_vendor.index.hideLoading();
            common_vendor.index.__f__("error", "at pages/appointment/detail.vue:824", "支付课程费失败:", error);
            common_vendor.index.showToast({
    ...
    // uni-pay 组件事件：订单创建成功
    onPayCreate(res) {
      common_vendor.index.__f__("log", "at pages/appointment/detail.vue:836", "支付订单创建成功:", res);
    },
    // uni-pay 组件事件：支付成功
    async onPaySuccess(res) {
      var _a, _b, _c;
      common_vendor.index.__f__("log", "at pages/appointment/detail.vue:840", "[支付成功] uni-pay 回调:", res);
      const isPaid = res.has_paid || res.status === 1 || res.user_order_success;
      if (!isPaid) {
        common_vendor.index.__f__("warn", "at pages/appointment/detail.vue:845", "[支付成功] 支付成功事件但状态异常:", res);
        return;
      }
      ...
      common_vendor.index.__f__("log", "at pages/appointment/detail.vue:855", "[支付成功] 订单信息:", {
        order_no,
        out_trade_no,
        order_id,
      ...
      try {
        ...
        if (!finalOrderNo) {
          common_vendor.index.__f__("log", "at pages/appointment/detail.vue:870", "[支付成功] 未找到 order_no，通过 appointment_id 查找订单...");
          ...
          if (orderListRes.code === 0 && orderListRes.data && orderListRes.data.list && orderListRes.data.list.length > 0) {
            ...
            common_vendor.index.__f__("log", "at pages/appointment/detail.vue:881", "[支付成功] 找到订单:", finalOrderNo);
          }
        }
        if (!finalOrderNo) {
          throw new Error("无法获取订单号，请稍后刷新页面查看支付状态");
        }
        common_vendor.index.__f__("log", "at pages/appointment/detail.vue:890", "[支付成功] 更新订单状态，order_no:", finalOrderNo);
        const payRes = await paymentCreate.mockPaySuccess({
        ...
        if (payRes.code === 0) {
          common_vendor.index.__f__("log", "at pages/appointment/detail.vue:898", "[支付成功] 数据库更新成功:", {
            appointment_status: (_b = payRes.data) == null ? void 0 : _b.appointment_status,
            order_no: finalOrderNo
          });
          if (out_trade_no) {
            common_vendor.index.__f__("log", "at pages/appointment/detail.vue:905", "[支付成功] out_trade_no 已通过 mockPaySuccess 保存:", out_trade_no);
          }
          if (this.appointment) {
            this.appointment.parent_paid = true;
        ...
        common_vendor.index.__f__("error", "at pages/appointment/detail.vue:927", "[支付成功] 更新数据库失败:", error);
        if (this.appointment) {
          this.appointment.parent_paid = true;
          this.appointment.status = this.appointment.status === "pending_payment" ? "pending_confirm" : this.appointment.status;
        }
    ...
    onPayFail(err) {
      common_vendor.index.__f__("error", "at pages/appointment/detail.vue:947", "支付失败:", err);
    ...
    async contactTeacher() {
      ...
      if ((_a = this.appointment) == null ? void 0 : _a.conversation_id) {
        try {
          ...
          common_vendor.index.__f__("error", "at pages/appointment/detail.vue:996", "检查消息失败:", error);
          common_vendor.index.navigateTo({
            url: `/pages/chat/conversation?conversationId=${this.appointment.conversation_id}`
          });
        }
      ...
      try {
        ...
        if (res.code === 0 && ((_b = res.data) == null ? void 0 : _b.conversation_id)) {
          try {
            ...
            common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1036", "检查消息失败:", error);
            common_vendor.index.navigateTo({
              url: `/pages/chat/conversation?conversationId=${res.data.conversation_id}`
            });
          }
        ...
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1052", "获取会话失败:", error);
        common_vendor.index.navigateTo({
    ...
    async handleRefund() {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1076", "[申请退款] 开始查找订单, appointment_id:", this.appointmentId);
        const paymentCreate = common_vendor.tr.importObject("payment-create", { customUI: true });
        ...
        common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1088", "[申请退款] 方法1-云对象查询结果:", {
          code: orderRes.code,
        ...
        common_vendor.index.hideLoading();
        if (orderRes.code !== 0) {
          common_vendor.index.__f__("warn", "at pages/appointment/detail.vue:1099", "[申请退款] 方法1查询失败，尝试方法2");
        ...
        if (orderRes.data && orderRes.data.list) {
          ...
          common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1120", "[申请退款] 方法1-找到的订单:", paidOrder ? {
        ...
        if (paidOrder) {
          try {
            ...
            if (refundRes.code === 0 && refundRes.data) {
              ...
              common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1136", "[申请退款] 已存在退款申请:", {
            ...
            common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1159", "[申请退款] 查询退款记录（无记录）:", error.message);
          }
          common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1163", "[申请退款] 跳转到退款页面, order_id:", paidOrder._id);
          common_vendor.index.navigateTo({
            url: `/pages/order/refund?id=${paidOrder._id}`
          });
        } else {
          common_vendor.index.__f__("warn", "at pages/appointment/detail.vue:1169", "[申请退款] 方法1未找到订单，尝试方法2-直接查询数据库", {
            appointment_id: this.appointmentId,
          ...
          try {
            ...
            common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1186", "[申请退款] 方法2-直接查询结果:", {
            ...
            if (directOrderRes.data && directOrderRes.data.length > 0) {
              ...
              common_vendor.index.__f__("log", "at pages/appointment/detail.vue:1211", "[申请退款] 方法2-找到订单:", {
              ...
              common_vendor.index.__f__("warn", "at pages/appointment/detail.vue:1224", "[申请退款] 方法2也未找到订单");
            }
          } catch (dbError) {
            common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1227", "[申请退款] 方法2-直接查询失败:", {
              error: dbError,
...
2. 联系客服处理`,
        ...
        common_vendor.index.hideLoading();
        common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1242", "[申请退款] 查找订单失败:", {
          error,
      ...
      common_vendor.index.showModal({
        ...
        success: async (res) => {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1282", "试课退款失败:", error);
      ...
      common_vendor.index.showModal({
        ...
        success: async (res) => {
          ...
          try {
            ...
            common_vendor.index.__f__("error", "at pages/appointment/detail.vue:1314", "确认课程完成失败:", error);

...
const _sfc_main = {
  ...
  methods: {
    async refreshData() {
      common_vendor.index.__f__("log", "at pages/common/register.vue:363", "[register] 下拉刷新：重新加载资料");
    ...
    async initPage(fromPullDown = false) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:373", "初始化家长资料失败:", error);
      } finally {
        if (fromPullDown) {
    ...
    async fetchProfile() {
      ...
      try {
        ...
        if (res.code === 0 && res.data) {
          ...
          common_vendor.index.__f__("warn", "at pages/common/register.vue:410", "获取用户信息失败，使用本地存储信息:", res.message);
          if (stored.uid) {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:423", "获取家长资料失败:", error);
    ...
    async uploadAvatar(localPath) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:534", "上传头像失败:", error);
    ...
    async getTempFileURL(fileId) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:548", "获取临时链接失败:", error);
        return fileId;
      }
    },
    onGradeChange(event) {
    ...
    async handleChooseLocation() {
      ...
      try {
        ...
        if (error.message && !error.message.includes("取消")) {
          common_vendor.index.__f__("error", "at pages/common/register.vue:626", "选择位置失败:", error);
          common_vendor.index.showToast({
    ...
    async submitForm() {
      if (!this.canEdit || this.isSubmitting) {
        common_vendor.index.__f__("log", "at pages/common/register.vue:685", "[register] 保存被阻止:", { canEdit: this.canEdit, isSubmitting: this.isSubmitting });
        return;
      }
      if (!this.validateForm()) {
        common_vendor.index.__f__("log", "at pages/common/register.vue:689", "[register] 表单验证失败");
        return;
      }
      try {
        this.isSubmitting = true;
        common_vendor.index.__f__("log", "at pages/common/register.vue:694", "[register] 开始保存，payload:", {
        ...
        common_vendor.index.__f__("log", "at pages/common/register.vue:731", "[register] 调用云函数 updateParentProfile");
        const userProfile = common_vendor.tr.importObject("user-profile", { customUI: true });
        const res = await userProfile.updateParentProfile(payload);
        common_vendor.index.__f__("log", "at pages/common/register.vue:734", "[register] 云函数返回:", res);
        if (res.code === 0) {
          try {
            const inviteCenter = common_vendor.tr.importObject("invite-center", { customUI: true });
            await inviteCenter.getMyInviteCode();
          } catch (e) {
            common_vendor.index.__f__("error", "at pages/common/register.vue:742", "[register] 生成邀请码失败（忽略，不影响资料保存）:", e);
          }
          const stored = common_vendor.index.getStorageSync("userInfo") || {};
          const parentInfo = {
            real_name: this.formData.real_name,
          ...
          common_vendor.index.setStorageSync("userInfo", nextStored);
          common_vendor.index.__f__("log", "at pages/common/register.vue:775", "[register] 保存成功，已更新本地存储");
          ...
          common_vendor.index.__f__("error", "at pages/common/register.vue:792", "[register] 保存失败:", res.message);
          common_vendor.index.showToast({ title: res.message || "保存失败", icon: "none", duration: 3e3 });
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/common/register.vue:796", "[register] 保存家长资料异常:", error);
        const errorMsg = error.message || error.errMsg || "保存失败，请稍后再试";
        common_vendor.index.showToast({ title: errorMsg, icon: "none", duration: 3e3 });
      } finally {
        this.isSubmitting = false;
        common_vendor.index.__f__("log", "at pages/common/register.vue:801", "[register] 保存流程结束，isSubmitting:", this.isSubmitting);
      }
    },
    goRolePage() {

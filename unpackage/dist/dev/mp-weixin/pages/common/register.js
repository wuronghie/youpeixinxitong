...
const _sfc_main = {
  ...
  methods: {
    async refreshData() {
      common_vendor.index.__f__("log", "at pages/common/register.vue:341", "[register] 下拉刷新：重新加载资料");
    ...
    async initPage(fromPullDown = false) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:351", "初始化家长资料失败:", error);
      } finally {
        if (fromPullDown) {
    ...
    async fetchProfile() {
      ...
      try {
        ...
        if (res.code === 0 && res.data) {
          ...
          common_vendor.index.__f__("warn", "at pages/common/register.vue:387", "获取用户信息失败，使用本地存储信息:", res.message);
          if (stored.uid) {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:400", "获取家长资料失败:", error);
    ...
    async uploadAvatar(localPath) {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:500", "上传头像失败:", error);
    ...
    async getTempFileURL(fileId) {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/common/register.vue:514", "获取临时链接失败:", error);
        return fileId;
      }
    },
    onGradeChange(event) {
    ...
    async handleChooseLocation() {
      ...
      try {
        ...
        if (error.message && !error.message.includes("取消")) {
          common_vendor.index.__f__("error", "at pages/common/register.vue:582", "选择位置失败:", error);
          common_vendor.index.showToast({
    ...
    async submitForm() {
      if (!this.canEdit || this.isSubmitting) {
        common_vendor.index.__f__("log", "at pages/common/register.vue:633", "[register] 保存被阻止:", { canEdit: this.canEdit, isSubmitting: this.isSubmitting });
        return;
      }
      if (!this.validateForm()) {
        common_vendor.index.__f__("log", "at pages/common/register.vue:637", "[register] 表单验证失败");
        return;
      }
      try {
        this.isSubmitting = true;
        common_vendor.index.__f__("log", "at pages/common/register.vue:642", "[register] 开始保存，payload:", {
        ...
        common_vendor.index.__f__("log", "at pages/common/register.vue:678", "[register] 调用云函数 updateParentProfile");
        const userProfile = common_vendor.tr.importObject("user-profile", { customUI: true });
        const res = await userProfile.updateParentProfile(payload);
        common_vendor.index.__f__("log", "at pages/common/register.vue:681", "[register] 云函数返回:", res);
        if (res.code === 0) {
          try {
            const inviteCenter = common_vendor.tr.importObject("invite-center", { customUI: true });
            await inviteCenter.getMyInviteCode();
          } catch (e) {
            common_vendor.index.__f__("error", "at pages/common/register.vue:689", "[register] 生成邀请码失败（忽略，不影响资料保存）:", e);
          }
          const stored = common_vendor.index.getStorageSync("userInfo") || {};
          const parentInfo = {
            real_name: this.formData.real_name,
          ...
          common_vendor.index.setStorageSync("userInfo", nextStored);
          common_vendor.index.__f__("log", "at pages/common/register.vue:721", "[register] 保存成功，已更新本地存储");
          common_vendor.index.showToast({ title: "保存成功", icon: "success" });
          setTimeout(() => {
            common_vendor.index.navigateBack({ delta: 1 });
          }, 1200);
        } else {
          common_vendor.index.__f__("error", "at pages/common/register.vue:727", "[register] 保存失败:", res.message);
          common_vendor.index.showToast({ title: res.message || "保存失败", icon: "none", duration: 3e3 });
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/common/register.vue:731", "[register] 保存家长资料异常:", error);
        const errorMsg = error.message || error.errMsg || "保存失败，请稍后再试";
        common_vendor.index.showToast({ title: errorMsg, icon: "none", duration: 3e3 });
      } finally {
        this.isSubmitting = false;
        common_vendor.index.__f__("log", "at pages/common/register.vue:736", "[register] 保存流程结束，isSubmitting:", this.isSubmitting);
      }
    },
    goRolePage() {
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    p: !$options.canEdit,
    q: $data.formData.phone,
    r: common_vendor.o(common_vendor.m(($event) => $data.formData.phone = $event.detail.value, {
      trim: true
    })),
    s: !$options.canEdit,
    t: $data.formData.contact_wechat,
    v: common_vendor.o(common_vendor.m(($event) => $data.formData.contact_wechat = $event.detail.value, {
      trim: true
    })),
    w: !$options.canEdit ? 1 : "",
    x: common_vendor.p({
    ...
    y: !$options.canEdit,
    z: $data.formData.student_name,
    A: common_vendor.o(common_vendor.m(($event) => $data.formData.student_name = $event.detail.value, {
      trim: true
    })),
    B: common_vendor.t($data.formData.student_grade || "请选择年级"),
    C: common_vendor.n($data.formData.student_grade ? "" : "text-light-muted"),
    D: $data.gradeOptions,
    E: $data.gradeIndex,
    F: common_vendor.o((...args) => $options.onGradeChange && $options.onGradeChange(...args)),
    G: !$options.canEdit,
    H: !$options.canEdit,
    I: $data.formData.student_age,
    J: common_vendor.o(common_vendor.m(($event) => $data.formData.student_age = $event.detail.value, {
      trim: true
    })),
    K: !$options.canEdit,
    L: $data.formData.school_name,
    M: common_vendor.o(common_vendor.m(($event) => $data.formData.school_name = $event.detail.value, {
      trim: true
    })),
    N: common_vendor.f($data.subjectOptions, (item, k0, i0) => {
      return {
    ...
    Q: common_vendor.f($data.goalOptions, (item, k0, i0) => {
      return {
        a: common_vendor.t(item),
        b: item,
    ...
    R: !$options.canEdit,
    S: $data.formData.extra_notes,
    T: common_vendor.o(common_vendor.m(($event) => $data.formData.extra_notes = $event.detail.value, {
      trim: true
    })),
    U: common_vendor.t($data.formData.extra_notes.length),
    V: !$options.canEdit ? 1 : "",
    W: common_vendor.p({
      headTitle: "学习目标"
    }),
    X: common_vendor.t($options.addressDisplay || "点击选择地址"),
    Y: common_vendor.o((...args) => $options.handleChooseLocation && $options.handleChooseLocation(...args)),
    Z: $data.formData.address.latitude && $data.formData.address.longitude
  }, $data.formData.address.latitude && $data.formData.address.longitude ? {
    aa: parseFloat($data.formData.address.latitude),
    ab: parseFloat($data.formData.address.longitude),
    ac: $options.mapMarkers,
    ad: common_vendor.o((...args) => $options.handleOpenLocation && $options.handleOpenLocation(...args))
  } : {}, {
    ae: !$options.canEdit ? 1 : "",
    af: common_vendor.p({
      headTitle: "上课地址"
    }),
    ag: common_vendor.t($data.isSubmitting ? "保存中..." : "保存信息"),
    ah: !$options.canEdit || $data.isSubmitting,
    ai: common_vendor.o((...args) => $options.submitForm && $options.submitForm(...args))
  });
}

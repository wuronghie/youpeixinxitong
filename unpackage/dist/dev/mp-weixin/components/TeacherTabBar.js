  ...
  methods: {
    bindChatPush() {
      ...
      this._onChatPush = (payload) => {
        common_vendor.index.__f__("log", "at components/TeacherTabBar.vue:125", "[TeacherTabBar] 收到 push，刷新角标", payload);
        this.loadUnreadChat();
      };
      this._onChatBadge = (count) => {
        common_vendor.index.__f__("log", "at components/TeacherTabBar.vue:129", "[TeacherTabBar] 收到 badge 事件", count);
    ...
    async loadUnreadChat() {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at components/TeacherTabBar.vue:162", "[TeacherTabBar] loadUnreadChat=", res);
        if (res.code === 0) {
    ...
    handleClick(item) {
      ...
      setTimeout(() => {
        common_vendor.index.redirectTo({
          url: item.path,
          fail: (err) => {
            common_vendor.index.__f__("warn", "at components/TeacherTabBar.vue:191", "redirectTo 失败，尝试使用 navigateTo:", err);
            common_vendor.index.navigateTo({
              url: item.path,
              fail: (navErr) => {
                common_vendor.index.__f__("error", "at components/TeacherTabBar.vue:196", "页面导航失败:", navErr);
              }
            });
          }
        });
